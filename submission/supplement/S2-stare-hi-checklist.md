...
| 6.1 | Study design | ✅ | Named in §5.3 ("This is a specification-and-feasibility study"); §2 and §3 establish the same framing through the objectives and the methods. |
...
| Synthea | build `2b0a55b` | base population generator; the run record (build, both seeds, generation parameters) is deposited under `ig/evaluation/synthea-out/metadata/`, and the 197 MB jar is identified by that build rather than redistributed |
